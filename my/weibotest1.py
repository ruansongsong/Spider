...
class CollectData():
  ...
  ##构建URL%25E5%25A4%25A9%25E6%25B4%25A5
  def getURL(self):
    return self.begin_url_per+self.getKeyWord()+"&region=custom:"+self.region+"&typeall=1&suball=1"+self.timescope+"&nodup=1&page="
        ...
        for line in lines:
          ...
          if line.startswith('<script>STK && STK.pageletM && STK.pageletM.view({"pid":"pl_weibo_direct"'):
            ...
            if n > 0:
              ...
              ## 没有更多结果页面
              if (j.find('<div class="feed_lists W_texta">') > 0):
                hasMore = False
              ## 有结果的页面
              else:
                page = etree.HTML(j)
                dls = page.xpath(u"//em")   #使用xpath解析
                for dl in dls:
